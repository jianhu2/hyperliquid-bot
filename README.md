...
这是一个包含了3个独立策略的 Hyperliquid 交易机器人项目。使用这些工具前，您必须具备 Python 基础知识，并完全理解其代码逻辑和潜在的财务风险。
...
3.  **您必须审查代码**：您有**绝对的责任**去阅读、理解并审计您计划运行的每一行代码（包括 `ds_copier_v2.py`, `follow_bot_v3.py` `btc_follow_bot_v1.py` 以及所有被引用的 `example_utils.py` 等辅助文件）。不理解代码就运行，无异于将您的资产交给一个完全不透明的黑箱。
...
#### `follow_bot_v3.py` 的关键参数(基于V1迭代)：

```
#基础配置
TARGET_USER_ADDRESS = "0x9263c1bd29aa87a118242f3fbba4517037f8cc7a"  # 跟单目标地址
MY_INVESTMENT_USD = 168.88    # 每次跟单投入金额(USD)
TAKE_PROFIT_USD = 4500.0      # 止盈目标金额(USD)
COIN = "ETH"                  # 监控的币种
LOOP_SLEEP_SECONDS = 30       # 检查间隔(秒)

# 风险控制参数
LIQUIDATION_WARNING_PERCENT = 10.0    # 预警阈值(%)
LIQUIDATION_DANGER_PERCENT = 3.5      # 安全阈值(%)
AUTO_CLOSE_PERCENT = 2.0              # 自动平仓阈值(%)
RISK_COOLDOWN_MINUTES = 5             # 风险冷却时间(分钟)
```
